...
private:
    ...
    inline bool isOver() const { return fail_cnt == 3; }
    inline bool isComplete() const { return correct_cnt == (cnt[level] + level_cnt); }
    inline bool isNew() const { return !correct_cnt && !fail_cnt; }
...
public:
    ...
    void click(int x, int y) {
        ...
        if (data[x][y]) score += (level * 30) - (level * 10 * fail_cnt);
    }
